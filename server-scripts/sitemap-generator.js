...
(async () => {
    ...
    const newSitemapIndexData = sitemapSubData.map(({ name }) => getIndexUrl(name))
...
function getStaticPath () {
    const staticPath = fs
        .readdirSync('pages')
        .filter((staticPage) => !['api', '_app.js', '_document.js', '404.jsx', 'thank.jsx'].includes(staticPage))
        .map((staticPagePath) => (staticPagePath !== 'index.jsx' ? staticPagePath.split('.')[0] : ''))
...
function createSubSitemap(fileName, data) {
    const sitemapSubStart = '<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">'
...
function request({ query, variables, includeDrafts = false, excludeInvalid = false }) {
    const headers = {
